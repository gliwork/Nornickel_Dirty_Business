...
def isSmearDetected(src):
    ...
    meanByPixel = np.zeros((1080,1920,3),np.float)
    progressBar = 0
    lastProg = 0
    for img in data:
        curr_image = cv2.imread(img)
        # resize all images to the same size i.e. 500x500
        resize_curr_image = cv2.resize(curr_image,(1920,1080))
    ...
    cv2.imwrite('Mean_video_frames.jpg', meanByPixel)
    ...
    if contours:
        ...
        if(cv2.contourArea(contours[0])>minArea and cv2.contourArea(contours[0])<maxArea):
            ...
            cv2.imwrite("final_video_frames.jpg",resizedRead)
...
if __name__ == "__main__":
    ...
    #Put all videoframes into the folder '/video/'
    ...
    if(isSmearDetected(path)):
        print ("Smear is detected for "+path+" source.")
    else:
        print("No Smear in "+ path)